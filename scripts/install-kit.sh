    ...
    if [ "${#SKILL_FILTERS[@]}" -eq 0 ]; then
      ...
      for filter in "${SKILL_FILTERS[@]}"; do
        ...
        if [ "$matched" -eq 0 ]; then
          echo "No skill file matched '$filter'. Use an exact portable skill name such as miica-plan, miica-fix-issue, miica-documentation, miica-analyse, or miica-implementation." >&2
          exit 1
        fi
      done
    fi
