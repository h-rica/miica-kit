...
$SkillFiles = if ($Skills.Count -gt 0) {
    foreach ($Skill in $Skills) {
        ...
        if (-not $Match) {
            throw "No skill file matched '$Skill'. Use the exact portable skill name, for example: miica-plan, miica-fix-issue, miica-documentation, miica-analyse, or miica-implementation."
        }
        $Match
    }
} else {
