...
if [ "${#SKILLS[@]}" -eq 0 ]; then
  ...
  for wanted in "${SKILLS[@]}"; do
    ...
    if [ ! -d "$match" ]; then
      echo "No Codex skill matched '$wanted'. Use exact names like miica-plan, miica-fix-issue, miica-documentation, miica-knowledge, miica-deep-dive, miica-analyse, or miica-implementation." >&2
      exit 1
    fi
    selected+=("$match")
  done
