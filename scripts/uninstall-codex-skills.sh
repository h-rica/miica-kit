...
if [ "${#SKILLS[@]}" -eq 0 ]; then
  ...
  for wanted in "${SKILLS[@]}"; do
    ...
    if [ ! -d "$match" ]; then
      echo "No Codex skill matched '$wanted'. Use exact names like miica-plan, miica-architecture, miica-fix-issue, miica-documentation, miica-knowledge, miica-deep-dive, miica-analyse, miica-review, miica-implementation, miica-git, or miica-execute-plan." >&2
      exit 1
    fi
    selected+=("$match")
  done
