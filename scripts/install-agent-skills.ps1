...
$Selected = if ($Skills.Count -gt 0) {
    foreach ($Skill in $Skills) {
        ...
        if (-not $Match) {
            throw "No agent skill matched '$Skill'. Use exact names like miica-plan, miica-fix-issue, miica-documentation, miica-knowledge, miica-deep-dive, miica-analyse, or miica-implementation."
        }
        $Match
    }
} else {
...
Write-Host "Installed agent skills to $TargetDir"
